...
export const sanitizeNotificationPath = (value = "") => {
  const raw = String(value || "").trim();
  if (!raw || raw === "#") return "";
  if (/^https?:\/\//i.test(raw)) return raw;
  return compactPath(raw.startsWith("/") ? raw : `/${raw}`);
};
...
const isGenericPath = (path = "") => genericPaths.has(String(path || "").split("?")[0]);
...
const resolveEntity = (notification = {}) => {
  ...
  return {
    direct: sanitizeNotificationPath(
      getValue(source, [
        "link",
        "url",
        "actionUrl",
...
export const enrichNotificationTarget = (notification = {}) => {
