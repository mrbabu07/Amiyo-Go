...
export default function VendorSupportChat() {
  ...
  const loadChat = async () => {
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/chat/vendor/my-chat`, {
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/chat/vendor/send`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50">
      {/* Header */}
      <div className="bg-gradient-to-r from-orange-500 to-orange-600 shadow-lg px-6 py-4">
        <div className="flex items-center gap-4">
          <Link to="/vendor/dashboard" className="text-white hover:text-orange-100">
          ...
          <div>
            ...
            <p className="text-sm text-orange-100">Get help with your shop, payouts, and more</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        {!chat?.messages || chat.messages.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">Start a conversation with the admin</p>
          </div>
        ) : (
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-xs font-semibold">
                    {msg.senderType === 'vendor' ? 'You' : 'Admin'}
                  </span>
                </div>
                ...
                <p className={`text-xs mt-2 ${msg.senderType === 'vendor' ? 'text-orange-100' : 'text-gray-500'}`}>
      ...
      <form onSubmit={handleSend} className="bg-white border-t border-gray-200 p-4 shadow-lg">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="p-3 text-gray-500 hover:text-orange-600 hover:bg-orange-50 rounded-lg transition"
            title="Attach files"
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button
            ...
            className="bg-orange-600 text-white px-6 py-3 rounded-lg hover:bg-orange-700 disabled:opacity-50 disabled:cursor-not-allowed font-semibold transition"
          >
            {sending ? (
